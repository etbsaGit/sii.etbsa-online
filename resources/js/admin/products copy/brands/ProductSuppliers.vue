<template>
  <div>
    <v-toolbar flat class="text-h4">
      <v-icon left x-large>mdi-tractor-variant</v-icon>Proveedores
      <v-spacer></v-spacer>
      <v-text-field
...
export default {
  ...
  methods: {
    async loadProductSupplier(cb) {
      ...
      await axios
        .get("/admin/products/suppliers", { params })
        .then((response) => {
    ...
    async submit() {
      ...
      if (_this.editedId === -1) {
        ...
        await axios
          .post("/admin/products/suppliers", _this.form)
          .then((response) => {
        ...
        await axios
          .put(`/admin/products/suppliers/${_this.editedId}`, _this.form)
