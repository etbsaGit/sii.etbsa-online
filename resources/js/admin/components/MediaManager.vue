<template>
  <div>
    <v-card>
      ...
      <v-list subheader two-line color="trasparent" dense>
        <v-list-item-group v-model="selected_files" multiple>
          ...
          <v-list-item
            v-for="(folder, index) in files.filter((f) => f.type == 'folder')"
            :key="`${folder.name}-${index}`"
            :value="folder"
            v-on:dblclick="openFile(folder)"
          >
            <v-list-item-avatar>
              <v-icon class="grey lighten-1"> mdi-folder </v-icon>
            </v-list-item-avatar>
          ...
          <v-list-item
            v-for="(file, index) in files.filter((f) => f.type != 'folder')"
            :key="`${file.name}-${index}`"
            :value="file"
    ...
    >
      <template #title> Subir Nuevo Archivo </template>
      <v-card flat>
        <v-file-input
    ...
    >
      <template #title> crear Nueva Carpeta </template>
      <v-card flat>
        <v-text-field
    ...
    >
      <template #title> Confimar Eliminacion Archivos/Carpetas </template>
      <v-card flat>
        <div class="title">
    ...
    >
      <template #title> Mover Archivo y/o Folder </template>
      <v-card flat>
        <v-select
