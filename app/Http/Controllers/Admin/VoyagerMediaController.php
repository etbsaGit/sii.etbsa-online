...
class VoyagerMediaController extends AdminController
{
    /** @var string */
    private $filesystem;
    ...
    public function get_file(Request $request)
    {
        $path = $request->path;
        // return dd($path);
        if (Storage::disk($this->filesystem)->exists($path)) {
            return Storage::disk($this->filesystem)->response($path);
        } else {
    ...
    public function rename(Request $request)
    {
        ...
        if (!Storage::disk($this->filesystem)->exists("{$location}/{$newFilename}")) {
            if (Storage::disk($this->filesystem)->move("{$location}/{$filename}", "{$location}/{$newFilename}")) {
                ...
                $error = 'media.error_moving';
            }
        } else {
            $error = 'media.error_may_exist';
        }
    ...
    //     $storage = Storage::disk($this->filesystem)->addPlugin(new ListWith());
    //     $storageItems = $storage->listWith(['mimetype'], $dir, true);
    //     foreach ($storageItems as $file) {
    //         // We're skipping all subfolders
    //         // if (!$file->isDir()) {
    //         $filePath =
    //             Storage::disk($this->filesystem)->response($file['path']);
